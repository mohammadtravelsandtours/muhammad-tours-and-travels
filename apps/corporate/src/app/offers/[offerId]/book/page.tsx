...
import { apiClient, ApiError, CostCenter, isPriceConfirmationRequired, newIdempotencyKey } from '@/lib/api-client';
...
export default function BookOfferPage() {
  ...
  useEffect(() => {
    ...
    apiClient
      .listMyCostCenters(accessToken)
      .then((r) => setCostCenters(r.costCenters))
      .catch(() => setCostCenters([])); // non-fatal: booking still works charged to the employee's own default cost center
  }, [accessToken]);
  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="max-w-3xl mx-auto px-6 pt-10 pb-24">
        ...
        {!loadError && !offer && <p className="text-sm text-graphite-500">Loading…</p>}

        {offer && (
          <>
            <ol className="flex items-center gap-2 text-xs text-graphite-500 mb-6">
              {STEPS.map((label, i) => (
                <li key={label} className={`flex items-center gap-2 ${i === step ? 'text-graphite-900 font-medium' : ''}`}>
                  <span className={`w-5 h-5 rounded-full flex items-center justify-center border ${i <= step ? 'bg-indigo text-white border-indigo' : 'border-line'}`}>
                    {i + 1}
                  </span>
                  {label}
                  {i < STEPS.length - 1 && <span className="text-graphite-500">—</span>}
                </li>
              ))}
            </ol>

            <div className="rounded-xl bg-white border border-line p-4 mb-6 text-sm text-graphite-700 flex flex-wrap items-center justify-between gap-2">
              <span>
                {routeSummary(legs)}
                {legs.map((leg, i) => (
                  <span key={i}>
              ...
              <span className="font-medium text-graphite-900">{formatMoney(offer.totalFare, offer.currency)}</span>
            </div>
              ...
              <div className="space-y-5">
                {passengers.map((p, i) => (
                  <div key={i} className="rounded-2xl bg-white border border-line shadow-sm p-5">
                    <p className="text-sm font-medium text-graphite-900 mb-3">
                ...
                <div className="flex justify-end">
                  <button
                    ...
                    className="rounded-lg bg-graphite-900 text-white font-medium px-6 py-2.5 hover:bg-graphite-700"
                  >
                    Continue
                  </button>
                </div>
            ...
            {step === 1 && (
              <div className="space-y-4">
                  ...
                  <div className="rounded-2xl bg-white border border-line shadow-sm p-5">
                    <Field label="Cost center">
                      <select
                        ...
                        className="w-full rounded-md border border-line px-3 py-2"
                      >
                        <option value="">Use my default cost center</option>
                        {costCenters.map((c) => (
                    ...
                    <p className="mt-2 text-xs text-graphite-500">
                      This booking will be charged against the cost center you pick here — leave it on the default to
                      use whatever cost center your profile is already assigned to.
                    </p>
                  </div>
                )}
                ...
                <div className="flex justify-between">
                  <button onClick={() => setStep(0)} className="text-sm text-graphite-500 hover:text-graphite-900">Back</button>
                  <button onClick={goToReview} className="rounded-lg bg-graphite-900 text-white font-medium px-6 py-2.5 hover:bg-graphite-700">
                    Review booking
                  </button>
                </div>
              </div>
            ...
            {step === 2 && (
              <div className="space-y-5">
                <div className="rounded-2xl bg-white border border-line shadow-sm p-5">
                  <p className="text-sm font-medium text-graphite-900 mb-2">Travelers</p>
                  <ul className="text-sm text-graphite-700 space-y-0.5">
                    {passengers.map((p, i) => (
                      <li key={i}>{p.title} {p.firstName} {p.lastName} · {p.type.toLowerCase()}</li>
                    ))}
                  </ul>
                  <p className="text-sm font-medium text-graphite-900 mt-4 mb-2">Contact</p>
                  <p className="text-sm text-graphite-700">{contactName} · {contactEmail} · {contactPhone}</p>
                  {costCenterId && costCenters && (
                    <>
                      <p className="text-sm font-medium text-graphite-900 mt-4 mb-2">Cost center</p>
                      <p className="text-sm text-graphite-700">
                        {costCenters.find((c) => c.id === costCenterId)?.code} — {costCenters.find((c) => c.id === costCenterId)?.name}
                      </p>
                    </>
                  )}
                </div>
                ...
                <div className="rounded-2xl bg-white border border-line shadow-sm p-5">
                  ...
                  {previewTotal !== null && previewTotal !== offer.totalFare && (
                    <p className="mt-2 text-xs text-indigo-dim">
                ...
                <p className="text-xs text-graphite-500 bg-base rounded-lg px-4 py-3">
                  This booking will be confirmed with the airline immediately, but tickets are only
                  issued once your approver reviews and approves it under Approvals.
                </p>
                ...
                {priceConfirm ? (
                  <div className="rounded-xl bg-base p-4 text-sm text-graphite-700">
                    <p>
                    ...
                    <div className="mt-3 flex gap-3">
                      <button
                        ...
                        className="rounded-lg bg-indigo text-white font-medium px-5 py-2 hover:bg-indigo-dim disabled:opacity-60"
                      >
                        {submitting ? 'Booking…' : `Accept ${formatMoney(priceConfirm.newTotal, priceConfirm.currency)} and book`}
                      </button>
                      <button onClick={() => setPriceConfirm(null)} className="text-sm text-graphite-500">
                        Cancel
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex justify-between">
                    <button onClick={() => setStep(1)} className="text-sm text-graphite-500 hover:text-graphite-900">Back</button>
                    <button
                    ...
                    >
                      {submitting ? 'Booking…' : 'Submit for approval'}
                    </button>
                  </div>
                )}
              </div>
...
function Field({ label, children }: { label: string; children: ReactNode }) {
  return (
    <label className="block">
      <span className="block text-xs text-graphite-500 mb-1">{label}</span>
      {children}
    </label>
  );
}
