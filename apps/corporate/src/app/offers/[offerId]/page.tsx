  ...
  return (
    <main className="min-h-screen">
      ...
      <section className="max-w-3xl mx-auto px-6 pt-10 pb-24">
        ...
        {!error && !offer && <p className="text-sm text-graphite-500">Loading fare details…</p>}

        {offer && (
          <>
            <div className="flex items-center justify-between">
              <h1 className="text-2xl font-semibold text-graphite-900">{routeSummary(legs)}</h1>
              {offer.isMock && (
                <span className="text-xs rounded-full bg-indigo/10 text-indigo-dim px-3 py-1 font-medium">
                  DEMO / MOCK fare
                </span>
              )}
            </div>
              ...
              {legs.map((leg, li) => (
                <div key={li}>
                  {legLabel(li, legs.length) && (
                    <p className="text-xs uppercase tracking-wide text-graphite-500 font-medium mb-2">
                  ...
                  <div className="rounded-2xl bg-white border border-line shadow-sm p-5 space-y-5">
                    {leg.segments.map((s, i) => (
                      <div key={i} className="flex gap-4">
                        <div className="text-xs text-graphite-500 w-20 shrink-0 pt-0.5">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between text-sm">
                            ...
                            <span className="text-graphite-500">{formatDuration(s.durationMinutes)}</span>
                            <span className="text-graphite-900 font-medium">
            ...
            <div className="mt-8 flex items-center justify-between rounded-2xl bg-white border border-line shadow-sm p-5">
              <div>
                <p className="text-xs text-graphite-500">Total price, all passengers</p>
                <p className="text-3xl font-semibold text-graphite-900">{formatMoney(offer.totalFare, offer.currency)}</p>
              </div>
              <button
                onClick={() => router.push(`/offers/${offer.id}/book`)}
                className="rounded-lg bg-indigo text-white font-medium px-6 py-3 hover:bg-indigo-dim"
              >
                Continue to booking
              </button>
            </div>
...
function InfoCard({ title, children }: { title: string; children: ReactNode }) {
  return (
    <div className="rounded-xl bg-white border border-line p-4">
      <p className="text-xs font-medium text-graphite-700">{title}</p>
      <p className="mt-1 text-sm text-graphite-500">{children}</p>
    </div>
  );
}
