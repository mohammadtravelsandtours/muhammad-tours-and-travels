          ...
          <>
            ...
            {TERMINAL_BAD.includes(booking.status) && (
              <p className="mt-4 text-sm text-red-600 bg-red-50 rounded-lg px-4 py-3">
                ...
                charged for a confirmed fare. Please search again or contact support.
              </p>
            )}

            {booking.offer && (
              <div className="mt-6 rounded-2xl bg-white border border-sand shadow-sm p-5">
                <p className="text-xs text-dusk-500 mb-2">
                ...
                {booking.offer.segments.map((s, i) => (
                  <div key={i} className="flex items-center justify-between text-sm py-1.5 border-t border-sand first:border-t-0 first:pt-0">
                    <span className="text-dusk-500 w-20 shrink-0">{s.marketingCarrier} {s.flightNumber}</span>
                    <span className="text-dusk-900">{formatDateTime(s.departureAt)} {s.origin}</span>
                    <span className="text-dusk-500">→</span>
                    <span className="text-dusk-900">{formatDateTime(s.arrivalAt)} {s.destination}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="mt-6 grid sm:grid-cols-2 gap-4">
              <div className="rounded-xl bg-white border border-sand p-4">
                <p className="text-xs font-medium text-dusk-700 mb-2">Travelers</p>
            ...
            {booking.tickets.length > 0 && (
              <div className="mt-4 rounded-xl bg-white border border-sand p-4">
                <p className="text-xs font-medium text-dusk-700 mb-2">Tickets</p>
                <ul className="text-sm text-dusk-500 space-y-0.5">
            ...
            <details className="mt-6 text-xs text-dusk-500">
              <summary className="cursor-pointer font-medium text-dusk-700">Booking history</summary>
            ...
            <Link href="/bookings" className="mt-8 inline-block text-sm text-tangerine-dim">
              ← Back to my bookings
            </Link>
          </>
        )}
