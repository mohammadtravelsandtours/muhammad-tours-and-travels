    ...
    <main className="min-h-screen">
      ...
      <section className="max-w-5xl mx-auto px-6 pt-10 pb-24">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Link href="/search" className="text-sm text-tangerine-dim">
            New search
          </Link>
        </div>
        ...
        {!error && !result && (
          <p className="mt-6 text-sm text-dusk-500">Loading results…</p>
        )}
        ...
        {result && result.offers.length > 0 && (
          <div className="mt-4 grid sm:grid-cols-[220px_1fr] gap-6 items-start">
            <aside className="rounded-xl border border-sand bg-white p-4 space-y-5 text-sm sm:sticky sm:top-4">
              <div>
                <p className="text-xs uppercase tracking-wide text-dusk-500 font-medium mb-2">Stops</p>
                <div className="space-y-1.5">
                  {(
                    ...
                    <label key={value} className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className={stopsFilter === value ? 'text-dusk-900' : 'text-dusk-500'}>{label}</span>
                    </label>
                  ))}
                </div>
              </div>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                <input
                  ...
                  className="accent-tangerine"
                />
                <span className="text-dusk-700">Refundable only</span>
              </label>

              {carriers.length > 1 && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs uppercase tracking-wide text-dusk-500 font-medium">Airline</p>
                    {carrierFilters.length > 0 && (
                      <button type="button" onClick={() => setCarrierFilters([])} className="text-xs text-tangerine-dim">
                        Clear
                      </button>
                    )}
                  </div>
                    ...
                    {carriers.map((c) => (
                      <label key={c} className="flex items-center gap-2 cursor-pointer">
                        <input
                          ...
                          className="accent-tangerine"
                        />
                        <span className={carrierFilters.includes(c) ? 'text-dusk-900' : 'text-dusk-500'}>{c}</span>
                      </label>
                    ))}
                  </div>
                </div>
            ...
            <div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-dusk-500">
                <span>Sort by:</span>
                {SORTS.map(({ key, label }) => (
                  <button
                    key={key}
                    onClick={() => setSort(key)}
                    className={sort === key ? 'text-dusk-900 font-medium underline' : 'hover:text-dusk-900'}
                  >
                    {label}
                  </button>
                ))}
              ...
              {offers.length === 0 ? (
                <p className="mt-4 text-sm text-dusk-500 bg-white border border-sand rounded-lg p-4">
