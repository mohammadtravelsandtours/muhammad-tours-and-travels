    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div className="flex-1 min-w-[240px]">
          <p className="text-xs text-dusk-500 mb-1.5">{offer.isMock ? 'DEMO fare' : offer.validatingCarrier}</p>
          <div className="space-y-2.5">
            {legs.map((leg, i) => (
              <div key={i}>
                {legLabel(i, legs.length) && (
                  <p className="text-[11px] uppercase tracking-wide text-dusk-500 font-medium">{legLabel(i, legs.length)}</p>
                )}
                <p className="text-lg font-medium text-dusk-900 leading-tight">
      ...
      <div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs text-dusk-500">
        ...
        {offer.isMock && <span className="text-tangerine-dim">DEMO / MOCK — not a real fare</span>}
      </div>

      {offer.transitVisaWarning && (
        <p className="mt-2 text-xs text-dusk-500 bg-sand/50 rounded-md px-3 py-2">
          ⚠ {offer.transitVisaWarning}
        </p>
      )}
    </Link>
