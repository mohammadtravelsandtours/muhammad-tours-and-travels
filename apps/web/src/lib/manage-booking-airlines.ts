/**
 * Airlines' own official "Manage Booking" pages, keyed by IATA code.
 *
 * This site never touches the reservation itself here — a PNR + last name
 * only ever gets typed on the airline's own site, in the tab this page
 * opens. That's a deliberate, honest limit: no arbitrary airline can be
 * reliably deep-linked with those values prefilled (most manage-booking
 * pages are JS-driven forms with no documented query-string API), so this
 * page hands the traveler their inputs to paste in rather than pretending
 * to submit on their behalf.
 *
 * Every URL below was verified (via web search, and where possible a
 * direct fetch) against the airline's own current site before being added.
 * Two carriers seeded elsewhere in this codebase (database/seeds/data/airlines.ts)
 * are deliberately left out of this list: Vistara (merged into Air India,
 * no longer operates its own booking site) and Jetstar Asia (ceased
 * operations in 2025). AirAsia is also left out — no stable, distinct
 * manage-booking URL on airasia.com could be confirmed with confidence.
 * Re-verify periodically; airlines redesign these pages without notice.
 */
