// Frontend-side mirrors of the shapes FlightsController/BookingsController
// actually serialize (see apps/api/src/modules/flights/flights.controller.ts
// and modules/bookings/bookings.controller.ts). Kept separate from
// @mohammad-travels/types because these are HTTP response DTOs (numbers
// already coerced from Prisma Decimal, nested include shapes) rather than
// the adapter-facing domain contract that package defines.
...
/**
 * Shape the mock supplier adapter fills in (see
 * mock-flight-supplier.adapter.ts) — real supplier adapters may omit this
 * entirely (`fareRules: undefined`), so every field here stays optional and
 * the UI falls back to the plain refundable/changeable booleans when absent.
 */
export interface OfferFareRules {
  /** Fee charged to change the travel date on this fare, in the offer's currency. 0 means free changes. */
  changeFee?: number | null;
  /** Fee retained on a refund/cancellation, in the offer's currency. null on non-refundable fares. */
...
export interface FlightOffer {
  ...
  legs: OfferLeg[];
  /** Only present on the single-offer detail view (GET /flights/offers/:id) — tells the booking form how many passenger records to collect. */
