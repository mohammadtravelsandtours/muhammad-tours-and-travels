...
export default function BookingDetailPage() {
  ...
  useEffect(() => {
    if (!authLoading && !user) router.replace(`/login?next=${encodeURIComponent(`/bookings/${params.id}`)}`);
        ...
        {booking && (
          <>
            <div className="flex items-center justify-between">
              <div>
                <h1 className="text-2xl font-semibold text-slate-900">Booking {booking.bookingReference}</h1>
                <p className="text-sm text-slate-500 mt-1">Created {formatDateTime(booking.createdAt)}</p>
              </div>
            ...
            {TERMINAL_GOOD.includes(booking.status) && (
              <p className="mt-4 text-sm text-slate-700 bg-base rounded-lg px-4 py-3">
                {booking.status === 'TICKETED'
                  ? 'The ticket has been issued. Details are below.'
                  : 'This booking is confirmed with the supplier. Ticketing will follow.'}
              </p>
            )}
            {TERMINAL_BAD.includes(booking.status) && (
              <p className="mt-4 text-sm text-red-600 bg-red-50 rounded-lg px-4 py-3">
                This booking did not complete ({BOOKING_STATUS_LABEL[booking.status].toLowerCase()}). Your agency wallet
                has not been charged for a confirmed fare. Please search again or contact support.
              </p>
            )}
              ...
              <div className="mt-6 rounded-2xl bg-white border border-line shadow-sm p-5">
                ...
                {booking.offer.segments.map((s, i) => (
                  <div key={i} className="flex items-center justify-between text-sm py-1.5 border-t border-line first:border-t-0 first:pt-0">
                    <span className="text-slate-500 w-20 shrink-0">{s.marketingCarrier} {s.flightNumber}</span>
                    <span className="text-slate-900">{formatDateTime(s.departureAt)} {s.origin}</span>
                    <span className="text-slate-500">→</span>
                    <span className="text-slate-900">{formatDateTime(s.arrivalAt)} {s.destination}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="mt-6 grid sm:grid-cols-2 gap-4">
              <div className="rounded-xl bg-white border border-line p-4">
                <p className="text-xs font-medium text-slate-700 mb-2">Travelers</p>
              ...
              <div className="rounded-xl bg-white border border-line p-4">
                <p className="text-xs font-medium text-slate-700 mb-2">Total debited from wallet</p>
                <p className="text-2xl font-semibold text-slate-900">{formatMoney(booking.totalAmount, booking.currency)}</p>
                <p className="text-xs text-slate-500 mt-1">{booking.contactEmail} · {booking.contactPhone}</p>
              </div>
            </div>

            {booking.tickets.length > 0 && (
              <div className="mt-4 rounded-xl bg-white border border-line p-4">
                <p className="text-xs font-medium text-slate-700 mb-2">Tickets</p>
                <ul className="text-sm text-slate-500 space-y-0.5">
            ...
            <details className="mt-6 text-xs text-slate-500">
              <summary className="cursor-pointer font-medium text-slate-700">Booking history</summary>
            ...
            <Link href="/bookings" className="mt-8 inline-block text-sm text-teal-dim">
              ← Back to agency bookings
            </Link>
          </>
        )}
      </section>
...
function StatusBadge({ status }: { status: Booking['status'] }) {
  ...
  const cls = good ? 'bg-green-100 text-green-700' : bad ? 'bg-red-100 text-red-700' : 'bg-line text-slate-700';
